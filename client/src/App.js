        ...
        <div className="h-screen w-40 bg-black"></div>
            <div className="max-w-md mx-auto space-y-8">
                ...
                <div className="flex max-w-md mx-auto justify-between">
                        ...
                        <button disabled = {imageData?false:true} onClick={()=> setImageData(prev=>`data:image/jpg;base64, ${invert(prev.substring(prev.indexOf(',')+1).trim())}`)} className="rounded-sm p-2 hover:bg-slate-900 hover:text-white transition-all shadow-sm shadow-slate-300 bg-slate-100">Invert</button>
                </div>
                <div className={`max-w-md mx-auto mt-8 w-96 h-64 ${imageData?'':'shadow-md border'} `}>
                    <img src={imageData} alt="There is no picture yet" className={`object-cover ${imageData?'':'hidden'}`}/>
                </div>
